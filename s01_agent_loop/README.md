...
本章用约 120 行、零依赖的 Node 实现一个最小 agent：一个 while 循环加一个 shell 工具。它是后续所有章节的基础。

## 什么是 agent

ChatGPT 的交互方式是：你说一句，它答一句，结束。这是 chatbot。

Claude Code 的交互方式是：你说"帮我修这个 bug"，它自己去读文件、跑测试、改代码、再跑测试，最后回来告诉你结果。这是 agent。
...
**决定循环继续还是停止的，是模型，不是代码。** 代码唯一的工作是：模型要用工具，就执行，然后把结果递回去。这就是"自主"的最小实现。Claude Code、Codex 等 coding agent 的主循环在拓扑上都是这一个。
...
这段 JSON 是写给模型看的说明书。模型读了 `description`，才知道存在一个叫 `run_shell` 的工具，以及参数格式。v01 只提供这一个工具，因为 shell 几乎覆盖了常见需求：能跑命令就能读文件、写文件、查环境、跑测试。
...
把 cwd 和平台放进 system prompt，是所有真实 agent 都做的事。模型如果不知道自己在哪台机器的哪个目录里，就会猜错路径、在 Windows 上跑 `ls`。
...
两个实际细节：`content` 和 `tool_calls` 可能同时存在（模型边说"我来看看目录"边调用工具），所以 content 要打印；`function.arguments` 是 JSON **字符串**不是对象，要 parse，而且 parse 失败也要回填错误而不是崩溃（网络不稳时模型可能吐出不完整的 JSON）。
...
- 这 120 行和 Claude Code 的主循环在拓扑上是同一个东西。真实产品多出来的几万行，都花在这个循环的周边：工具更多更可靠（s02）、不空转（s03）、上下文不爆（s04/s06）、便宜（s07）、断了能接上（s08）、能分身（s09）。循环本身从 s01 到最后一章不再修改。
- 桌面 agent [Reina](https://github.com/Reina-Agent/Reina) 的引擎（`runTurn`）也是这个循环。它的 system prompt 同样以 cwd + 平台开头。
