...
几千 token，每轮都付。更阴的是 s07 讲过的第二笔账：**tools 数组紧跟 system 之后，是 prompt cache
前缀的一部分**——数组一变，前缀就断，后面全按全价重算。
...
工具名而数组不增长，是靠 OpenAI Responses 的服务端 `namespace_tools`——那是 provider 特权，
...
`docs/progressive-tool-disclosure-plan.md` 里规划、对标 Claude Code 的方向。Claude Code 正是这么做的：
`CORE_TOOLS` 白名单常驻，其余走 deferred；被发现的工具**永不回灌**广播数组，调用经一个稳定的
代理工具（`ExecuteExtraTool`）派发——tools 数组全程恒定，客户端自管。本章的"好做法"就是它。
