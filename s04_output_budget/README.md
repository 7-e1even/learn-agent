...
s03 的循环预算管"跑多少轮"，本章管"每轮往上下文里放多少"：给工具输出设预算，超预算的部分完整落盘、只留指针，做到不丢信息。

本章代码 = s03 基底 + 溢出机制模块 [spill.mjs](./spill.mjs)。

## 问题：大输出的三种坏结局

你让 agent 检查一个导出文件，它执行 `cat data.json`——一个 2MB 的压缩 JSON。接下来是三种坏结局之一：

1. 撑爆窗口：2MB 约等于五十多万 token，直接超出上下文窗口，API 返回 400。
2. 持续计费：就算窗口够大（1M 窗口的模型确实装得下），这五十万 token 从此留在历史里，之后每一轮调用都重新计费一遍。一次 cat，成本翻倍。
3. 截断丢信息：s02 给 `read_file` 加过一个 50KB 截断。但截掉的部分模型永远看不到，它也不知道自己错过了什么。如果问题恰好在第 51KB，就查不出来了。
...
三个要素缺一不可：**在哪**（路径），**多大**（模型据此决定要不要读、分几段读），**怎么取**（read_file + offset/limit，外加一句"不要重跑命令"——没有这句，模型的第一反应往往是把 cat 再跑一遍）。这延续了 s02 的原则：报错和提示都是写给模型看的界面。指针写清楚，模型照做就能取回细节；不需要就省下了 token。

### ② 预算要两层：单条上限 + 整轮总量

只设"单条不超过 50KB"防不住：模型一轮发起十个工具调用，每条 30KB，都不超单条上限，加起来 300KB 照样超窗。所以预算是两层的，溢出按 largest-first 执行：
...
从最大的开始处理效率最高：溢出一条 60KB 通常就能让整轮回到预算内，剩下三条 30KB 原文保留（细节还在上下文里，模型不用去翻文件）。达标就停——预算的目的是保住窗口，不是压缩所有输出。

这层聚合预算在真实产品里还有一个更现实的动机：MCP 这类外部工具的输出不经过你的单条截断——外部工具不受你控制，聚合预算是它们唯一的兜底。

### ③ read_file 是特例：文件本身就是指针

`enforceTurnBudget` 跳过了 `spillable: false` 的条目。`read_file` 读出来的内容本来就在磁盘上，再落盘一份副本是浪费（Reina 早期版本犯过这个错，后来修掉了）。它的正确形状是自我设限：
...
`offset/limit` 分段读取，尾注告诉模型总量和续读位置。同一个思想的两种形态：内存里的大输出 → 落盘 + 指针；磁盘上的大文件 → 指针就是它自己。最终所有大内容都收敛到同一个动作：read_file，分段读取。
...
两条保险规则：行数太少不处理（一份 20 行的干净结果没有压缩价值）；省不到 15% 就原样返回（为 3% 的收益打乱原文格式不划算）。宁可多留几行噪声，也不折叠掉一行真错误。

压缩会不会丢信息？不会——折叠真的发生时，全文同样落盘、同样给指针。压缩决定"回给模型多少"，落盘保证"内容完整保存"，整条链路仍然无损。
...
- **引擎层**（`packages/core/src/engine.ts` 的 `enforceTurnObservationBudget`）：每个请求前跑一遍整轮观测预算——单条超 100,000 字符、或整轮总量超 200,000 字符时 largest-first 溢出（小窗口模型按窗口的 15% / 30% 缩放）；替换成头尾节选 + `.reina/tool_outputs/<callId>.txt` 指针。这层正是为了兜住"单条上限管不到的聚合"和"完全不经过截断的 MCP 工具输出"。另有一个 24,000 字符的紧急裁剪，只在请求即将超出窗口时启用——那是有损的最后手段，但它也带指针，不留死路。
