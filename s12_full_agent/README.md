...
本章代码 = s01 的循环 + s03–s10 各章模块的**直接 import**（一行机制代码都不重写）+ 一台照剧本念台词的假模型服务器。这是主线的最后一章，也是一次验收：前面十一章吹过的牛——"循环写完之后一行不改，所有机制围着它长"——现在要当场兑现。
...
而"循环没变过"不是修辞。s01 的 `runTurn` 是这样的：
