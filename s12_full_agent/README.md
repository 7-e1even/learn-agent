...
前面各章的模块单独都验证过，可拼到一起呢？压缩（s06）要整体改写历史，持久化（s08）却坚持只追加；崩溃修复（s05）往内存插消息，落盘对此一无所知。本章把 s01 的循环和 s03–s10 的模块直接 import 进同一个 agent，用一台按剧本回复的假模型服务器做 17 项端到端验收——顺便兑现承诺：循环写完不再改，机制都挂在外围。
